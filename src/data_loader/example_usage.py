...
def main() -> None:
    ...
    images, masks, valid_masks, metas = next(iter(weak_loader))
    print(images.shape, masks.shape, valid_masks.shape, metas[0])

    # --- Loss masking pattern (copy this into your training loop) ---
    # criterion = torch.nn.BCEWithLogitsLoss(reduction='none')
    # pred = model(images)                          # [B, 1, H, W]
    # loss_px = criterion(pred, masks.float())      # [B, 1, H, W]
    # vm = valid_masks.unsqueeze(1)                 # [B, 1, H, W]
